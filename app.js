(function (Chart) {
  ...
  */
  ...
  ChartJs.prototype.render = function (time, model) {
    this.model = model;
    this.chart.data.datasets[0].data = this.model;
    this.chart.update();
  };
  ...
  function FpsHistoryChart(view) {
    this.model = null;
    this.history = [];
    this.chart = new ChartJs(view, {
      type: 'line',
      data: {
        datasets: [{
          label: 'кдр/с',
    ...
    this.element.classList.add('fps-history-chart');
    this.maxClouds = 0;
    this.maxFps = 0;
  }

  FpsHistoryChart.prototype.render = function (time, model) {
    if (this.history.length === 100) {
      this.history = this.history.filter(function (item, index, array) {
        return index === 0 || index >= array.length / 2;
      })
    }
    if (this.history.length === 0 && model.fps === 0) {
      return;
    }
    if (this.maxFps < model.fps) {
      this.maxFps = model.fps;
    }
    if (model.clouds <= this.maxClouds) {
      return;
    }
    this.history.push({
      x: this.maxClouds,
    ...
    this.chart.render(time, this.history);
    this.maxClouds = model.clouds;
    this.maxFps = 0;
  };
  ...
  function SpeedometerChart(view) {
    this.value = 0;
    this.max = 0;
  ...
  SpeedometerChart.prototype.render = function (time, model) {
    var newValue = Math.floor(model.fps);
    if (this.value === newValue) return;
    this.value = newValue;
    this.max = Math.max(this.max, this.value);
    this.chart.render(time, [
      ...
      return [
        ...
        new View(
          ...
          function (dashboard) {
            return [
              new RenderOnTimer([
                new SpeedometerChart(dashboard),
                new FpsHistoryChart(dashboard)
              ], 1000)
            ];
          }
        )
